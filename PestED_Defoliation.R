...
##' @param X = [leaf, wood, root, storage, SOM, soil water, stem density, defense]  # CM:adding defense pool here
##' @param params
...
SEM <- function(X, params, inputs, pest, timestep=1800){ 
  ...
  ## root rot: factor acceleration of root turnover
  ...
  #  4 - minimum leaf & root
  #      roots:leaves constant (such that for climatic mean water balance, maintain E[supply/demand]~1)
  #      but if stressed, allowed to lose leaves
...
### paramters
timestep = 1800 # seconds, 1800 = 30 mins
params = list()
## hydrology
