...
alloc_defol_5pc_result <- list()
alloc_defol_10pc_result <- list()
alloc_defol_15pc_result <- list()
...
for (i in 1:length(defense_alloc_percent)){
  ...
  alloc_defol_5pc_result[[name]] <- defol_5pc
  alloc_defol_10pc_result[[name]] <- defol_10pc
  alloc_defol_15pc_result[[name]] <- defol_15pc
...
time_series_data <- SEM_plot_data_fx(alloc_defol_5pc_result) |>
...
# making plot data from alloc_turnover results list:
time_series_data <- SEM_plot_data_fx(alloc_defol_10pc_result) |>
  # add defense alloc values for labeling:
  mutate(alloc = defense_alloc_percent[model_run])
...
# making plot data from alloc_turnover results list:
time_series_data <- SEM_plot_data_fx(alloc_defol_15pc_result) |>
  # add defense alloc values for labeling:
  mutate(alloc = defense_alloc_percent[model_run])
